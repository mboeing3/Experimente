...
public class KnotenKomparator {
	...
	private double ebenenexponenttreffer = 0d;
	private double ebenenexponentkeintreffer = 0d;
	...
	public KnotenKomparator(int maximaleAuswertungsEbene,
			double ebenenexponenttreffer, double ebenenexponentkeintreffer) {
		super();
		this.maximaleAuswertungsEbene = maximaleAuswertungsEbene;
		this.ebenenexponenttreffer = ebenenexponenttreffer;
		this.ebenenexponentkeintreffer = ebenenexponentkeintreffer;
	}
	...
	public double getEbenenexponenttreffer() {
		return ebenenexponenttreffer;
	}

	public void setEbenenexponenttreffer(double ebenenexponenttreffer) {
		this.ebenenexponenttreffer = ebenenexponenttreffer;
	}

	public double getEbenenexponentkeintreffer() {
		return ebenenexponentkeintreffer;
	}

	public void setEbenenexponentkeintreffer(double ebenenexponentkeintreffer) {
		this.ebenenexponentkeintreffer = ebenenexponentkeintreffer;
	}
	...
	public Double vergleiche(Knoten k1, Knoten k2) {
		...
		double[] trefferWert = this.ermittleKnotenTrefferwert(verschmolzenerBaum, 3, 0d, 0d);
	...
	public double[] ermittleKnotenTrefferwert(Knoten knoten) {
		return this.ermittleKnotenTrefferwert(knoten, 0, this.maximaleAuswertungsEbene,this.ebenenexponenttreffer, this.ebenenexponentkeintreffer);
	}
	 ...
	 * @param ebenenexponenttreffer Exponent der Ebenennummer, welche Faktor fuer die Wertung von Trefferknoten ist.
	 * @param ebenenexponentkeintreffer Exponent der Ebenennummer, welche Faktor fuer die Wertung von Nicht-Trefferknoten ist.
	 * @return double-Array mit Trefferwert auf Index 0, Gesamtwert auf Index 1.
	 */
	public double[] ermittleKnotenTrefferwert(Knoten knoten, int maxebene, double ebenenexponenttreffer, double ebenenexponentkeintreffer) {
		return this.ermittleKnotenTrefferwert(knoten, 0, maxebene,
				ebenenexponenttreffer, ebenenexponentkeintreffer);
	}
	 ...
	 * @return double-Array mit Trefferwert auf Index 0, Gesamtwert auf Index 1.
	 */
	private double[] ermittleKnotenTrefferwert(Knoten knoten, int ebene, int maxebene, double ebenenexponenttreffer, double ebenenexponentkeintreffer) {
		...
		if (ebene > 0){
			...
			if (knoten.isMatch()) {
				...
				wert = knoten.getZaehler() * Math.pow(ebene, ebenenexponenttreffer);
				knotenMatches[0] += wert;
			} else {
				wert = knoten.getZaehler() * Math.pow(ebene, ebenenexponentkeintreffer);
			}
			// Zaehlerwert zur Gesamtzahl addieren
			knotenMatches[1] += wert;
		}
