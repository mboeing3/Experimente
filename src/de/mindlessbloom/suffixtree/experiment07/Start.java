...
/*
 * Baut Praefix- und Suffix-Tree (nicht -Trie!) aus dem Kontext jedes Vergleichsworts
 * verwendet Fliesskommazahl als Knotenberuhrungszaehler (inkrement pro Beruehrung
 * wird pro Entfernungsschritt des hinzugefuegten Suffixes halbiert)
 * und errechnet in bekannter Weise den Uebereinstimmungsquotienten.
 */
import java.io.File;
...
public class Start {
	...
	public static void main(String[] args) throws Exception {
		...
		// Nur den mit dem Vergleichswort beginnenden Zweig des jeweiligen Suffixbaumes zum Vergleich heranziehen.
		boolean praefixBaumErstellen = (kommandozeile.hasOption("P"));
		...
		for (int i=0; i<vergleichWorte.length; i++){
			...
			int gefundeneVorkommen = baumBauer.baueBaumAusSaetzenMitWorttyp(vergleichWorte[i], satzListe, wortFilter[i], suffixBaumWurzelKnotenListe.get(i), praefixBaumWurzelKnoten, praefixBaumErstellen, ausfuehrlicheMeldungen, maximaleKontextEntfernungVonWort, true);
